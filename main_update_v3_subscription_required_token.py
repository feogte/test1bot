...
BOT_TOKEN = "8838387309:AAH2qawFJv4irZvROcPmpc6fcqZS_XHCnOQ"

TERMS_VERSION = 2
if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN не задан в переменных окружения Bothost")
...
# Пользовательское соглашение для новых пользователей.
TERMS_URL = "https://telegra.ph/usloviya-polzovaniya-vinex-shop-08-09"

DEFAULT_TEXTS = {
    "menu": "<b>Приветствуем вас в Vinex shop! ❤️‍🔥</b>\nздесь вы можете приобрести разные товары",
    "store": "<b>Store 🏪</b>\n\nВыберите раздел магазина:",
    "faq": "<b>Как происходит оплата?</b>\n\nВы оплачиваете товар рублями/звездами, после чего нажимаете кнопку <b>«отправить подтверждение»</b>, после этого аккаунт выдается.",
    "support": "Нажмите кнопку ниже, чтобы открыть чат с поддержкой.",
    "reviews": "Отзывы покупателей находятся в отдельном Telegram-канале.",
    "terms": "<b>Добро пожаловать в Vinex shop! ❤️‍🔥</b>\n\nНажимая на кнопку снизу, вы соглашаетесь с <a href=\"https://telegra.ph/usloviya-polzovaniya-vinex-shop-08-09\">условиями пользования</a>.",
}
...
def parse_quick_product_line(value: str) -> tuple[str, int, int, str, str, int] | None:
    # Простой формат физов: +57 | 50₽ | 5 шт
...
async def render_user_screen(
    ...
    **kwargs: Any,
    ...
    """Всегда отправляет НОВОЕ пользовательское сообщение.

    Старые сообщения никогда не редактируются и не удаляются.
    Это специально сделано для навигации по inline-кнопкам.
    """
...
async def send_bot_image(
    ...
    """Всегда отправляет новое сообщение и не трогает предыдущие."""
    clean_image = image_source(image)
    if clean_image and len(text) <= 1024:
        try:
            return await bot.send_photo(
...
def main_menu_keyboard(show_admin: bool = False) -> ReplyKeyboardMarkup:
    # FAQ — отдельная широкая кнопка; Store и Reviews — рядом; Support — широкая снизу.
    rows = [
        [KeyboardButton(text="FAQ ℹ️")],
        [KeyboardButton(text="Store 🏪"), KeyboardButton(text="Reviews 📨")],
...
def admin_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(text="📝 Изменить тексты", callback_data="admin:texts")
    builder.adjust(1)
...
class Database:
    ...
    async def initialize(self) -> None:
        async with self.lock:
            ...
            if "terms_accepted" not in user_columns:
                ...
                # Пользователи, существовавшие до обновления, не считаются новыми.
                self.conn.execute("UPDATE users SET terms_accepted = 1")
    ...
    async def get_text(self, key: str) -> str:
        return await self.get_setting(f"text_{key}", DEFAULT_TEXTS.get(key, ""))

    async def set_text(self, key: str, value: str) -> None:
        await self.set_setting(f"text_{key}", value)

    async def set_terms_accepted(self, user_id: int, accepted: bool = True) -> None:
        async with self.lock:
            self.conn.execute(
                "UPDATE users SET terms_accepted = ?, terms_version = ?, last_seen_at = ? WHERE user_id = ?",
                (
                    1 if accepted else 0,
                    TERMS_VERSION if accepted else 0,
                    utc_now_iso(),
                    user_id,
                ),
            )
    ...
    async def terms_accepted(self, user_id: int) -> bool:
        async with self.lock:
            row = self.conn.execute(
                "SELECT terms_accepted, terms_version FROM users WHERE user_id = ?",
                (user_id,),
            ).fetchone()
            return bool(
                row
                and row["terms_accepted"]
                and int(row["terms_version"] or 0) >= TERMS_VERSION
            )
        ...
        async with self.lock:
            try:
                ...
                if product["category"] == "accounts":
                    ...
                    # Счётчик stock уже зарезервирован при создании заказа.
                    # Отсутствие записи account_stock не должно блокировать кнопку
                    # «Выдать». Если номер есть — привязываем его; если нет —
                    # подтверждаем заказ без кнопки получения номера.
                    if account:
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            or "query is not modified" in message
        ):
            logger.debug("Callback уже устарел/обработан: %s", exc)
            return False
...
class SubscriptionMiddleware(BaseMiddleware):
    async def __call__(
        ...
        # /start всегда должен быть доступен: новый пользователь сначала принимает
        # условия пользования, а уже затем получает обычный интерфейс магазина.
        if isinstance(event, Message) and (event.text or "").split()[0:1] == ["/start"]:
            return await handler(event, data)

        # Кнопка проверки подписки должна быть доступна даже неподписанному пользователю.
        if isinstance(event, CallbackQuery) and event.data in {"subscription:check", "terms:accept"}:
            return await handler(event, data)
...
async def callback_check_subscription(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if subscribed:
        ...
        await state.clear()
        if callback.message:
            if not await db.terms_accepted(callback.from_user.id):
                terms_text = await db.get_text("terms")
                await answer_user_message(
                    callback.message,
                    terms_text,
        ...
        if callback.message:
            await send_subscription_required(callback.message)


def terms_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✅ Я согласен", callback_data="terms:accept")],
        ]
    )


@router.callback_query(F.data == "terms:accept")
async def callback_terms_accept(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not is_owner(callback.from_user.id):
        ...
        if not subscribed:
            await safe_callback_answer(
                callback,
                "Сначала подпишитесь на обязательный канал.",
                show_alert=True,
            )
            if callback.message:
                await send_subscription_required(callback.message)
    ...
    await safe_callback_answer(callback, "Условия приняты!")
    await state.clear()
    if callback.message:
        text = await db.get_text("menu")
        await send_section_message(
...
async def command_start(message: Message, state: FSMContext) -> None:
    ...
    if message.from_user and not is_owner(message.from_user.id):
        ...
        await db.set_subscription_verified(message.from_user.id, subscribed)
        if not subscribed:
            await send_subscription_required(message)
            return

    if message.from_user and not await db.terms_accepted(message.from_user.id):
        terms_text = await db.get_text("terms")
        await answer_user_message(message, terms_text, reply_markup=terms_keyboard())
        return
...
@router.message(F.text.in_({"FAQ ℹ️"}))
async def menu_faq(message: Message) -> None:
    await db.upsert_user(message)
    await send_section_message(message, "menu", await db.get_text("faq"), reply_markup=main_menu_keyboard(bool(message.from_user and is_owner(message.from_user.id))))


@router.message(F.text.in_({"Store 🏪", "store 🛍️"}))
...
@router.message(F.text.in_({"Reviews 📨", "Отзывы 📭", "📝 Отзывы"}))
async def menu_reviews(message: Message) -> None:
    ...
    await send_section_message(
        message,
        "reviews",
        await db.get_text("reviews"),
        reply_markup=keyboard,
    )


@router.message(F.text.in_({"Support 🎧", "Поддержка 🎧", "👨‍💻 Поддержка"}))
...
@router.callback_query(F.data.in_({"pay:card", "pay:stars", "payment_method:card", "payment_method:stars"}))
async def callback_payment_method(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    method = callback_data.split(":", 1)[1] if ":" in callback_data else ""
...
async def edit_admin_order_message(callback: CallbackQuery, order: sqlite3.Row) -> None:
    """Не редактирует заявку. Отправляет владельцу отдельное сообщение о результате."""
    try:
        ...
        logger.exception("Не удалось отправить владельцу новое сообщение по заказу #%s", order["id"])
...
async def command_texts(message: Message, state: FSMContext) -> None:
    if not message.from_user or not is_owner(message.from_user.id):
        await answer_user_message(message, "❌ У вас нет доступа к редактированию текстов.")
        return
    await state.clear()
    await answer_user_message(
        message,
        "<b>📝 Изменение текстов</b>\n\nВыберите сообщение:",
...
TEXT_EDIT_LABELS = {
    ...
    "support": "Support",
    "reviews": "Reviews",
    "terms": "Условия для новых пользователей",
}


def admin_texts_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for key, label in TEXT_EDIT_LABELS.items():
        builder.button(text=f"📝 {label}", callback_data=f"admin:text:{key}")
...
async def callback_admin_texts(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_owner(callback.from_user.id):
        await safe_callback_answer(callback, "Нет доступа.", show_alert=True)
        return
    await safe_callback_answer(callback)
    ...
    if callback.message:
        await answer_user_message(
            callback.message,
            "<b>📝 Изменение текстов</b>\n\nВыберите сообщение, текст которого хотите изменить.",
            reply_markup=admin_texts_keyboard(),
        )


@router.callback_query(F.data.startswith("admin:text:"))
async def callback_admin_text_edit(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_owner(callback.from_user.id):
        await safe_callback_answer(callback, "Нет доступа.", show_alert=True)
        return
    key = callback.data.split(":", 2)[2]
    if key not in TEXT_EDIT_LABELS:
        await safe_callback_answer(callback, "Неизвестный текст.", show_alert=True)
        return
    ...
    if callback.message:
        await answer_user_message(
            callback.message,
            f"<b>Изменение: {html.escape(TEXT_EDIT_LABELS[key])}</b>\n\n"
            f"Текущий текст:\n<blockquote>{current}</blockquote>\n\n"
            "Отправьте новый текст одним сообщением. HTML-разметка Telegram поддерживается.\n"
            "Для отмены: /cancel",
            reply_markup=admin_menu_keyboard(),
        )


@router.message(AdminStates.waiting_text_value, F.text)
async def process_admin_text_value(message: Message, state: FSMContext) -> None:
    if not message.from_user or not is_owner(message.from_user.id):
        return
    value = message.text.strip()
    if not value:
        await answer_user_message(message, "Текст не может быть пустым.")
        return
    if len(value) > 4096:
        await answer_user_message(message, "Текст слишком длинный. Максимум — 4096 символов.")
        return
    data = await state.get_data()
    key = str(data.get("edit_text_key", ""))
    if key not in TEXT_EDIT_LABELS:
        await state.clear()
        await answer_user_message(message, "❌ Неизвестный текст.", reply_markup=admin_menu_keyboard())
        return
    await db.set_text(key, value)
    await state.clear()
    await answer_user_message(
        message,
        f"✅ Текст «{html.escape(TEXT_EDIT_LABELS[key])}» сохранён.",
        reply_markup=admin_menu_keyboard(),
    )
...
async def callback_new_product(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.message:
        await answer_user_message(callback.message, 
            ...
            "<b>Физ:</b>\n"
            "<code>+57 | 50₽ | 5 шт</code>\n"
            "Добавит товар +57 в категорию аккаунтов с указанным остатком.\n\n"
            "<b>Пакет Stars:</b>\n"
...
async def set_commands() -> None:
    ...
    owner_commands = [
        ...
        BotCommand(command="texts", description="Изменить тексты бота"),
        BotCommand(command="cancel", description="Отменить текущее действие"),
    ]
